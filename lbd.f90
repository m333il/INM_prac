program lbd_algorithm
    ...
    integer :: n
    integer :: i, j
    double precision, dimension(:), allocatable :: A, AT, B, alpha, betta, tU, tV, UT, VT, U, V, v1, UUT, VVT, eye
    ...
    allocate(UUT(0 : n * n - 1), VVT(0 : n * n - 1))
    call matmul(U, UT, UUT, n)
    call matmul(V, VT, VVT, n)

    allocate(eye(0 : n * n - 1))
    eye = 0.0
    do i = 0, n - 1
        eye(i * n + i) = 1.0
    end do
...
function dot(M, a_ind, b_ind, n) result (res)
    implicit none
    double precision :: res
    double precision, allocatable, dimension(:), intent(in) :: M
    integer, intent(in) :: n, a_ind, b_ind
    integer :: i
    res = 0.0

    do i = 0, n - 1
        res = res + M(a_ind + i) * M(b_ind + i)
    end do
end function dot
